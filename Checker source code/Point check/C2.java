...
public class C2 {
	...
	public static boolean detectFieldH(String path){
		...
		try{
			...
			while((line = br.readLine()) != null) {
				...
				if(isPrivate && (line.contains("int*") || line.contains("int *"))) isPointer = true;
			}
			isOk = isClass && isPointer;
			if (isOk) { 
				//System.out.println(path);
				...
				if (!isPointer) bufWrongPath += nim +"; tidak menggunakan pointer "+ System.lineSeparator();
			}
			fis.close();
		} catch (Exception e){
			e.printStackTrace();
	...
	public static void main(String[] args) {
        ...
        try{
        	File rfile = new File("right_output_c2.txt");
        ...
        try{
        	File wfile = new File("wrong_output_c2.txt");
