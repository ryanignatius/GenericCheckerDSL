	...
	public static boolean detectFieldH(String path){
		...
		try{
			...
			while((line = br.readLine()) != null) {
				...
				if(isPrivate && (line2.contains("char*Name") || line2.contains("char*name"))){
					if (!isUsed[1]){
						isUsed[1] = true;
						score += 5;
					}
				}
				if(isPrivate && line2.contains("char[") && (line2.contains("]Name") || line2.contains("]name"))){
					if (!isUsed[1]){
						isUsed[1] = true;
						score += 5;
					}
				...
				if(isPrivate && (line2.contains("Person*FriendOf") || line2.contains("Person*friendOf"))){
					if (!isUsed[4]){
						isUsed[4] = true;
						score += 5;
					}
				...
				if(line2.matches("(.*)Person\\(string(.*),int(.*),Person*(.*)\\)(.*)")){
					if (!isUsed[6]){
						isUsed[6] = true;
						score += 10;
					}
				}
				if(line2.contains("Person(constPerson&")){
					if (!isUsed[7]){
						isUsed[7] = true;
						score += 10;
					}
				...
				if(line2.contains("Person&operator=(constPerson&")){
					if (!isUsed[9]){
						isUsed[9] = true;
						score += 10;
					}
				}
				if(line.matches("(.*)friend ostream& operator<<(\\s*)\\((\\s*)ostream&(.*),(\\s*)(.*)Person&(.*)\\)(.*)")){
					if (!isUsed[10]){
						isUsed[10] = true;
						score += 15;
					}
				}
				if(line.matches("(.*)friend istream& operator>>(\\s*)\\((\\s*)istream&(.*),(\\s*)(.*)Person&(.*)\\)(.*)")){
					if (!isUsed[11]){
						isUsed[11] = true;
						score += 15;
					}
